...
module FacebookGoogleCalendarSync
  ...
  class GoogleCalendar
    ...
    def initialize details, data, acl
      @details = details
      @data = data
      @acl = acl
    end
    ...
    def calendar_owner_email_addresses
      @acl.items.collect(&:scope).collect(&:value).select{ |email_address| email_address.end_with?("gmail.com") }
    end
    ...
    def self.calendar_with_events google_calendar_details
      ...
      acl =  get_acl(google_calendar_details.id)
      GoogleCalendar.new(google_calendar_details, events, acl)
    end
